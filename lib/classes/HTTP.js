...
class HTTP extends Kubik {
   ...
   * @return {Promise}
   */
  async _applyHooks(name) {
    const hookName = '_' + name;
    if (!this[hookName].length) return;
  ...
  async _scan() {
    ...
    for (const volume of this.volumes) {
      await helpers.scandir(volume, (file) => {
  ...
  listen(port, bind, protocol = 'http') {
    if (protocol === 'https') {
      ...
      https.createServer(this.httpApp).listen(+port, bind);
      return this;
    }
    this.httpApp.listen(+port, bind);
    return this;
  }
  ...
  use(extension) {
    if (!extension) return;
    if (isFunction(extension.before)) this._before.push(extension.before);
    if (isFunction(extension.after)) this._after.push(extension.after);
  ...
  async after() {
    ...
    if (Array.isArray(this.config.servers)) {
      for (const server of this.config.servers) {
        ...
        this.listen(server.port, server.bind, server.protocol);
        ports.push(`${bind}:${this.config.port}`);
      }
    }
    if (!ports.lengts) {
...
HTTP.prototype.CREATE_HTTP_APP = false;
module.exports = HTTP;
